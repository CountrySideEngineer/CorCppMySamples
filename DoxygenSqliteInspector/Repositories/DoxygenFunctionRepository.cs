...
{
    public async Task<Metum?> GetMetaAsync(string dbPath, CancellationToken cancellationToken = default)
    {
        await using var context = new DoxygenContext(dbPath);
        return await context.Meta.AsNoTracking().FirstOrDefaultAsync(cancellationToken);
    }
